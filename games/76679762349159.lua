...
InfoNotification("Bedwarz gaming :moneyface:", 1)
...
function store:getSword()
	if entitylib.isAlive then
		if (not lplr:findFirstChild("HotbarFolder")) then errorNotification("Failure fetching sword! Folder not found.", 3) end
		local items = lplr:FindFirstChild("HotbarFolder"):GetChildren() 
		for i,v in pairs(items) do
			local att = v:GetAttribute("ItemName")
...
function bedwarz:Get(path, root)
    ...
    if remote then
		...
		errorNotification("Bedwarz", "Failure grabing remote: "..tostring(path), 5)
        return {
...
local run = function(func)
	task.spawn(function()
		...
		if (not suc) then errorNotification("bedwarz", 'Failure executing function: '..tostring(err), 3); warn(debug.traceback(tostring(err))) end
	end)
end
